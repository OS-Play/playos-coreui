...
// source from flutter engine: flutter/shell/platform/common/json_method_codec.cc
...
JsonMethodCodec &JsonMethodCodec::instance()
{
    static JsonMethodCodec codec;
...
JsonMethodCodec::DecodeMethodCallInternal(const uint8_t* message,
  ...
  return std::make_unique<::flutter::MethodCall<rapidjson::Document>>(
...
std::unique_ptr<std::vector<uint8_t>> JsonMethodCodec::EncodeMethodCallInternal(
  ...
  return JsonMessageCodec::instance().EncodeMessage(message);
}
...
JsonMethodCodec::EncodeSuccessEnvelopeInternal(
  ...
  return JsonMessageCodec::instance().EncodeMessage(envelope);
}
...
bool JsonMethodCodec::DecodeAndProcessResponseEnvelopeInternal(
  ...
  std::unique_ptr<rapidjson::Document> json_response =
      JsonMessageCodec::instance().DecodeMessage(response, response_size);
  if (!json_response) {
    return false;
  }
