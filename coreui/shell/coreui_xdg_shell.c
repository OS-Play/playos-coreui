...
static void xdg_toplevel_map(struct wl_listener *listener, void *data) {
    ...
    coreui_view_focus(view, view->view.xdg_toplevel->base->surface);
}
...
static void begin_interactive(struct coreui_view *view,
    ...
    if (view->view.xdg_toplevel->base->surface !=
...
static void xdg_toplevel_request_move(
    ...
    struct coreui_view *view = wl_container_of(listener, view, request_move);
...
static void xdg_toplevel_request_resize(
    ...
    struct coreui_view *view = wl_container_of(listener, view, request_resize);
...
static void xdg_toplevel_request_maximize(
    ...
    wlr_xdg_surface_schedule_configure(view->view.xdg_toplevel->base);
}
...
static void server_new_xdg_surface(struct wl_listener *listener, void *data) {
    ...
    struct coreui_view *view =
        calloc(1, sizeof(struct coreui_view));
    view->server = server;
    view->view.xdg_toplevel = xdg_surface->toplevel;
    view->type = COREUI_VIEW_XDG_SHELL;

    view->scene_tree = wlr_scene_xdg_surface_create(
            server->layer_shell->app, view->view.xdg_toplevel->base);
    view->scene_tree->node.data = view;
    xdg_surface->data = view->scene_tree;
    xdg_surface->surface->current.viewport.src.x = 64;
